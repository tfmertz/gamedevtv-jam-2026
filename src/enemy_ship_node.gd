...
func _on_rand_timer_timeout() -> void:
	if rand_mode:
		...
		if sector_counter == -2:
			queue_free()
		target_location = Vector2(randi_range((wind_x*sector_counter/3+sprite_half),wind_x*(sector_counter+1)/3-sprite_half), randi_range(sprite_half,wind_y-sprite_half))
